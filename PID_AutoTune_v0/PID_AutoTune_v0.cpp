...
{
    return (ExtendedRuntime() == ATUNE_FINISHED) ? 1 : 0;
}

int PID_ATune::ExtendedRuntime()
{
    justevaled = false;
    if (peakCount > 9 && running)
    {
        running = false;
        FinishUp();
        return ATUNE_FINISHED;
    }
    unsigned long now = millis();

    if ((now - lastTime) < sampleTime)
        return ATUNE_RUNNING_NOOP;
    lastTime = now;
    double refVal = *input;
    justevaled = true;
    if (!running)
    ...
    { //we don't want to trust the maxes or mins until the inputs array has been filled
        return ATUNE_RUNNING_NEW_OUTPUT;
    }
